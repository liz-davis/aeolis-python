...
def initialize (s,p):
    ...
    s['germinate'][:,:] = False
    s['lateral'][:,:] = False
    s['bgbiomass'][:,:] = np.zeros_like(s['rhoveg'])
    #print("Initialized bgbiomass:", s['bgbiomass'])
...
def grow (s, p): #DURAN 2006
    ...
    # if p['process_tide']:     #COMMENTED OUT TO TEST!
...
def bgbiomass(s, p):
    '''
    Estimate the belowground biomass of vegetation based on aboveground vegetation density and distribute it evenly within the top 1.5 m of the grid.
    
    Parameters
    ----------
    s : dict
        Spatial grids containing vegetation data.
    p : dict
        Model configuration parameters.
    ...
    -------
    dict
        Updated spatial grids with belowground biomass data.
    '''
    # Retrieve 'rhoveg'
    # Validate and retrieve 'rhoveg'
    if 'rhoveg' not in s or s['rhoveg'] is None:
        raise ValueError("The 'rhoveg' key is missing in the spatial grids.")
    elif not np.any(s['rhoveg']):
        print("Warning: 'rhoveg' contains no vegetation density!")
    
    veg_cover = s['rhoveg']*100                          # Drew's equation is a whole number percent, not 0.1-0.9 like in aeolis
    #print("Vegetation cover (rhoveg):", veg_cover)

    # Initialize belowground biomass
    belowground_biomass = np.zeros_like(veg_cover)
    #print("Initialized belowground biomass:", belowground_biomass)
    
    # Calculate belowground biomass based on relationship between aboveground cover and total belowground biomass (White et al., 2024)
    mask = veg_cover > 0 
    if np.any(mask):
        try:
            belowground_biomass[mask] = np.exp(3.77 + 0.88 * np.log(veg_cover[mask] + 1)) - 1 # adding 1 to avoid log(0)
        except Exception as e:
            logger.error(f"Error computing bg biomass: {e}")
            raise
    
    #print("Computed belowground biomass:", belowground_biomass)
            
     
    # Log computation results for debugging
    #logger.debug(f"Belowground biomass array: {belowground_biomass}")
    # Calculate the depth of each grid cell
    # integrated_depth = 1.5  # Total depth for distribution
    # number_of_layers = s['rhoveg'].shape[0]  # Use the size of the first dimension of rhoveg as number of layers
    # layer_depth = integrated_depth / number_of_layers  # Depth of each layer

    # Distribute the biomass evenly across the top 1.5 m NOT YET
    s['bgbiomass'] = belowground_biomass # Update `s` with distributed biomass
