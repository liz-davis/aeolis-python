    ...
    if p['process_avalanche']:
        ...
        if initiate_avalanche:
            ...
            for i in range(0,max_iter_ava):
                ...
                #ix = np.logical_and(grad_h > tan_dyn, grad_h_nonerod > 0)... OLD DIDN't WORK FOR LIZ
                ix = grad_h > tan_dyn
                ...
                #print(f"→ slope_diff after 2nd condition: max = {np.max(slope_diff):.6f}, min = {np.min(slope_diff):.6f}")			                    
                ...
                if ny == 1:
                    ...
                    #print(f"Max flux_down: {np.max(np.abs(flux_down)):.6e}")
                    ...
                    net_flux = E * (q_in - q_out)
                    #print(f"[Avalanche] Max net_flux before bed update: {np.max(np.abs(net_flux)):.6e}")
